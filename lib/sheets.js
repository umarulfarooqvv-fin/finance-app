...
/** Append a new row (columns A–E only). Optional tab targets e.g. the income tab. */
export function appendRow({ tab, timestamp, amount, method, category, remarks }) {
  return call('append', { tab, timestamp, amount, method, category, remarks });
}
...
/** All rows (cols A–E) of a tab, as displayed strings. Immune to UI filters. */
export function getData(tab) {
  return call('getData', { tab });
}

/** Key-value config stored in the AppConfig tab (e.g. card settings). */
export function setConfig(key, value) {
  return call('setConfig', { key, value });
}

export function getConfig(key) {
  return call('getConfig', { key });
}
