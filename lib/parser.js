...
// ---------------------------------------------------------------------------
// Timestamp parsing — THREE formats coexist in the sheet:
...
//   C) "06-July-2024,  Saturday"          (date-only + weekday; the format the
//      Timestamp column is DISPLAYED in for almost the whole history — 2,456 of
//      2,460 rows as of Jul-2026. Missing this made the entire history import
//      land with ts = NULL, which blanked the date column and starved every
//      date-based analytic. The weekday is redundant and ignored.)
...
const FMT_A =
  /^\s*(\d{1,2}),\s+([A-Za-z]+)\s+(\d{2})\s+at\s+(\d{1,2}):(\d{2}):(\d{2}):(\d{2})\s*(AM|PM)\s*$/i;
...
export function parseTimestamp(raw) {
  ...
  if (m) {
    ...
    const dateOnly =
      hh === '12' && mi === '00' && ss === '00' && cc === '00' && ampm.toUpperCase() === 'AM';
    return {
      date: new Date(
        2000 + parseInt(yy, 10),
        month,
  ...
  if (m) {
    ...
    const month = MONTHS[monthName.toLowerCase()];
    if (month === undefined) return null;
    // No time component in this format — midnight, flagged date-only.
    return { date: new Date(parseInt(yyyy, 10), month, parseInt(dd, 10)), dateOnly: true };
  }
  return null;
}
...
export function normalizeEntry({ amount, method, category, remarks, date = null }, opts = {}) {
  ...
  const base = crypto
    .createHash('sha1')
    .update([ts, amt, m, c, r].join('|'))
    .digest('hex')
    .slice(0, 16);
  const id =
    opts.id ||
    'tx-' + base + (opts.deterministic ? '' : '-' + crypto.randomBytes(3).toString('hex'));
  return {
    id,
    ts,
...
export function normalizeRows(csvRows) {
  ...
  for (let i = 1; i < csvRows.length; i++) {
    ...
    if (!tsRaw.trim() && !amtRaw.trim() && !method.trim() && !category.trim() && !remarks.trim())
      continue;
