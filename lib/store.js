 ...
 * Supabase/PostgREST caps every response at 1000 rows server-side, so large
 * reads MUST paginate — we loop offset pages until a short page comes back.
 */
export async function select(table, opts = {}) {
  ...
  const page = 1000; // server-side max-rows cap per request
  const out = [];
  for (let off = 0; out.length < want; off += page) {
    const ask = Math.min(page, want - out.length);
    const q = new URLSearchParams(p);
    q.set('limit', String(ask));
    q.set('offset', String(off));
    const rows = await rest(`${table}?${q.toString()}`);
    out.push(...(rows || []));
    if (!rows || rows.length < ask) break; // short page → no more rows
  }
  return out;
}
