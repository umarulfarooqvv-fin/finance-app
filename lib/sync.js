...
// ---------------------------------------------------------------------------
/**
 * Ids of rows already marked verified in Postgres. The importer upserts with
 * resolution=merge-duplicates, so every column it sends overwrites the stored
 * value — sending a blanket `verified: false` would wipe the user's reconcil-
 * iation ticks. We read the verified ids first and re-send true for those.
 * Only verified rows are fetched (a small set), not the whole table.
 */
async function verifiedIds() {
  try {
    const { select, storeEnabled } = await import('./store.js');
    if (!storeEnabled()) return new Set();
    const rows = await select('transactions', { select: 'id', filters: { verified: 'eq.true' } });
    return new Set((rows || []).map((r) => r.id));
  } catch {
    return new Set(); // store unreachable → fall back to the safe default
  }
}

export async function sheetRowsForImport() {
  const txFetched = await fetchTabRows(TAB);
  const alreadyVerified = await verifiedIds();
  const transactions = normalizeRows(txFetched.rows).map((t) => ({
    ...
    tags: t.tags || {},
    verified: alreadyVerified.has(t.id), // preserve existing ticks; new rows → false
