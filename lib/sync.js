...
/**
 * Fetch a tab's rows as a 2D array WITH a synthetic header at index 0
 * (so row index i ↔ sheet row i+1, like the CSV shape).
 *
 * Primary source: Apps Script getData — returns ALL rows regardless of any
 * filter active in the sheet UI. (The CSV export silently drops rows hidden
 * by filters, which once hid ~2,000 historical rows.) Falls back to CSV when
 * the Apps Script isn't configured/reachable.
 */
async function fetchTabRows(tab) {
  const { writesEnabled, getData } = await import('./sheets.js');
  if (writesEnabled()) {
    try {
      const { rows } = await getData(tab);
      return { rows: [['Timestamp', 'B', 'C', 'D', 'E'], ...rows], source: 'apps-script' };
