...
/** Latest statement end (day-of-month billDate) that is <= now, plus surrounding cycle dates. */
export function cycleDates(billDate, graceDays, now = new Date()) {
  ...
  const dueDate = new Date(end.getFullYear(), end.getMonth(), end.getDate() + graceDays, 23, 59, 59, 999);
...
export function cardStatement(card, now = new Date()) {
  const db = getDb();
  const { statementEnd, cycleStart, dueDate } = cycleDates(card.bill_date, card.grace_days, now);
  const nowIso = iso(now), endIso = iso(statementEnd);

  const sum = (where, params) =>
    db.prepare(`SELECT COALESCE(SUM(amount),0) s FROM transactions WHERE deleted=0 AND amount IS NOT NULL AND ${where}`).get(...params).s;
  ...
  const billedClosing = debtThroughEnd - paidThroughEnd;
  ...
  const openingBalance = debtThroughStart - paidThroughStart;
