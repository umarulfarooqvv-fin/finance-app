...
test('forecast, EMI, budgets, holdings, net worth', async (t) => {
  ...
  try {
    ...
    await t.test('holdings: savings + investment totals and invoices', async () => {
      ...
      assert.equal(totals.savings, 50000);
      assert.equal(totals.investment, 23000);
      assert.equal(totals.invested, 70000);
      assert.equal(totals.currentValue, 73000);
      assert.equal(totals.gain, 3000);
      await deleteHolding(inv.id, db);
      assert.equal(listHoldings(db).totals.investment, 0);
