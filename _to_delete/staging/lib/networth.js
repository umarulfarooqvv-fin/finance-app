...
export function netWorthSnapshot(db = getDb(), now = new Date()) {
  ...
  const liabilities = { cardDebt: r2(cardDebt) };
  const totalAssets = r2(assets.bank + assets.savings + assets.investments + assets.creditGivenOutstanding);
  const netWorth = r2(totalAssets - liabilities.cardDebt);

  return { assets, liabilities, totalAssets, totalLiabilities: liabilities.cardDebt, netWorth };
}
...
export function netWorthTrend(db = getDb(), now = new Date()) {
  ...
  while (y < endY || (y === endY && m <= endM)) {
    ...
    const bank = openingSum + incomeToEnd.get(...accountNames, endIso).s - outflowToEnd.get(...accountNames, endIso).s;
    const cardDebt = cardDebtToEnd.get(endIso).s;
    const creditOut = Math.max(0, givenToEnd.get(endIso).s - returnsToEnd.get(endIso).s);
    const net = bank - cardDebt + creditOut + holdingsFlat;
