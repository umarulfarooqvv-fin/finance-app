  ...
  return (
    <>
      ...
      <div className="panel" style={{ borderColor: 'var(--accent)' }}>
        ...
        <div className="value accent" style={{ fontSize: 30, fontWeight: 800, margin: '4px 0', color: snapshot.netWorth < 0 ? 'var(--red)' : 'var(--green)' }}>
          {inr(snapshot.netWorth)}
        </div>
        <p className="muted small" style={{ margin: 0 }}>
          Assets {inr0(snapshot.totalAssets)} − card debt {inr0(snapshot.totalLiabilities)}
        </p>
      </div>
        ...
        <div style={{ width: '100%', height: 300 }}>
          <ResponsiveContainer>
            <LineChart data={trend} margin={{ top: 8, right: 16, bottom: 8, left: 8 }}>
              ...
              <YAxis stroke="#8b97a8" fontSize={11} tickFormatter={(v) => `${Math.round(v / 1000)}k`} width={44} />
              <Tooltip contentStyle={{ background: '#161b23', border: '1px solid #2a3342', borderRadius: 8 }} formatter={(v) => inr(v)} />
              <Line type="monotone" dataKey="netWorth" stroke="#5b8cff" strokeWidth={2.5} dot={false} name="Net worth" />
              <Line type="monotone" dataKey="bank" stroke="#34d399" strokeWidth={1.5} dot={false} name="Bank" />
              <Line type="monotone" dataKey="cardDebt" stroke="#f87171" strokeWidth={1.5} dot={false} name="Card debt" />
            </LineChart>
          </ResponsiveContainer>
        </div>
